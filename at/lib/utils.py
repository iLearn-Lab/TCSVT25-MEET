...
def update_codes_ICM(output, code, C, max_iter_update_b, N, M, K):
    with cp.cuda.Device(device_id):
        if isinstance(output, torch.Tensor):
            output = cp.asarray(
                output.detach().view(output.shape[0], -1).to(torch.device(f"cuda:{device_id}")).contiguous()
            )
        ...
        if isinstance(C, torch.Tensor):
            C = cp.asarray(
                C.detach().to(torch.device(f"cuda:{device_id}")).contiguous()
            )
        ...
        if isinstance(code, torch.Tensor):
            code = cp.asarray(
                code.detach().to(torch.device(f"cuda:{device_id}")).contiguous()
            )
        ...
        code = torch.as_tensor(code, device=torch.device(f"cuda:{device_id}"))
