...
def encode_data_train(model, data_loader,img_output ,txt_output):
    ...
    for i, (images, captions, lengths, ids) in enumerate(data_loader):
        ...
        _,_,img_emb,cap_emb = model.eval_emb(images, captions, lengths) #torch.Size([128, 1, 64])
...
def encode_data(model, data_loader, log_step=10, logging=print):
    ...
    for i, (images, captions, lengths, ids) in enumerate(data_loader):
        ...
        img_emb,cap_emb,cp_img_out,cp_txt_out = model.eval_emb(images, captions, lengths)
        if img_embs is None:
...
def afterhash_i2t(img_embs, cap_embs, id_list, fold5 = False):
...
def evalrank_cam(model_path,data_path,id_list_i2t,id_list_t2i,split='test', fold5 = False):
    ...
    print('vocabsize', opt.vocab_size )
    model = VSEModel(opt)
    ...
    data_loader = get_test_loader(data_path, opt.data_name,split, vocab, opt.batch_size, opt.workers, opt)
    ...
    img_embs = img_embs[::5]
    start = time.time()
    (r1i, r5i, r10i) = afterhash_i2t(img_embs, cap_embs, id_list_i2t)
    i2t_end = time.time()
    (r1t, r5t, r10t) = afterhash_t2i(img_embs, cap_embs,  id_list_t2i)
    t2i_end = time.time()
    print("evalution.py 489 calculate afterhash matching time i2t_need t2i_need:",i2t_end-start, t2i_end - i2t_end)
        
    return r1i+r5i+r10i+r1t+r5t+r10t, r1i, r5i, r10i, r1t, r5t, r10t, i2t_end-start, t2i_end - i2t_end, time2-time1
...
# def evalrank_clip(id_list_i2t,id_list_t2i):
#     # clip_model_path = LIRONG_ROOT / "data" / "clip-vit-base-patch32"
#     image_feature_path = DATA_ROOT / "clip_features_test" / "f30k" / "image_features.npy"
#     text_feature_path = DATA_ROOT / "clip_features_test" / "f30k" / "text_features.npy"
#     img_embs = np.load(image_feature_path)
#     cap_embs = np.load(text_feature_path)
#     img_embs = np.squeeze(img_embs)
#     cap_embs = np.squeeze(cap_embs)
#     print('image,cap',img_embs.shape,cap_embs.shape)
...
def evalrank_test(model_path,data_path,cam_model_path,split='test', fold5=False):
    ...
    hq_opt.vocab_path = str(VOCAB_ROOT)
    ...
    print('Gnerating hash and quantization vectors...')
    ...
    hash_idx_i2t = HammingD(cp_img_out, cp_txt_out, R_rate=0.6)
    # hash_idx_i2t = HammingD(cp_img_out, cp_txt_out, R_rate=1)
    hash_i2t_end = time.time()
    hash_idx_t2i = HammingD(cp_txt_out, cp_img_out, R_rate=0.7)
    ...
    q_i2t_end = time.time()
    q_idx_t2i = AQD_t2i(C_img, cp_txt_out, img_val_q, hash_idx_t2i, q_rate=0.7)
    q_end = time.time()
