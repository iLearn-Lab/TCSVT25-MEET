...
LIRONG_ROOT = Path(__file__).resolve().parent.parent
DATA_ROOT = LIRONG_ROOT / "data"
ESA_ROOT = LIRONG_ROOT / "ESA"
AT_ROOT = LIRONG_ROOT / "at"
...
def main():
    ...
    opt.logger_name = str(DATA_ROOT / "testmodel_logs")

    opt.H = 64
    opt.M = 8
    opt.K = 8
    hpara = "H" + str(opt.H) + "_M" + str(opt.M) + "_K" + str(opt.K)
    opt.model_name = str(AT_ROOT / "modelzoos" / "try1" / opt.data_name.split('_')[0] / hpara)
    ...
    # Load data loaders
    train_loader, val_loader,train_len,val_len = get_loaders(opt.data_name, vocab, opt.batch_size, opt.workers, opt)
    test_loader,test_len = get_test_loader_data("test", opt.data_name, vocab,
    ...
    for epoch in range(opt.num_epochs):
        ...
        # img_output,txt_output,a,b = encode_data(model, train_loader)
...
def validate(opt, val_loader, model,C_img,C_txt):
    ...
    if opt.data_name == "f30k_precomp":
        MODEL_PATH = str(ESA_ROOT / "f30k_butd_region_bigru_514.7" / "model_best.pth")
        currscore, r1i, r5i, r10i, r1t, r5t, r10t, afterhashtime_i2t, afterhashtime_t2i  = evalrank_cam(MODEL_PATH, DATA_PATH,q_idx_i2t, q_idx_t2i, split="test")
    else:
        print('coco')
        MODEL_PATH = str(ESA_ROOT / "coco_butd_region_bigru_525.3_429.5" / "model_best.pth")
        currscore, r1i, r5i, r10i, r1t, r5t, r10t, afterhashtime_i2t, afterhashtime_t2i = evalrank_cam(MODEL_PATH,
                                                                                                       DATA_PATH,
                                                                                                       q_idx_i2t,
                                                                                                       q_idx_t2i,
                                                                                                       split="testall")
