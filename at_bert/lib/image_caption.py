...
def process_caption(tokenizer, tokens, train=False,prob_i=0.2):
    output_tokens = []
    deleted_idx = []
...
def get_loader(data_path, data_name, data_split, tokenizer, opt, batch_size=100,
               shuffle=False, num_workers=2, train=False):
...
def get_test_loader(data_path, data_name, split_name, tokenizer, batch_size, workers, opt):    
    
    test_loader = get_loader(data_path, data_name ,split_name, tokenizer, opt, batch_size, False, workers, train=False)
