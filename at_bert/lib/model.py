...
from lib.encoders import get_image_encoder_hq, get_text_encoder_hq
...
class HQ(object):
    ...
    def __init__(self, opt):
        ...
        self.img_enc = get_image_encoder_hq(opt, opt.img_dim, opt.embed_size, no_imgnorm=opt.no_imgnorm)
        # self.txt_enc = EncoderText(opt.vocab_size, opt.word_dim,
        #                            opt.embed_size, opt.num_layers,
        #                            no_txtnorm=opt.no_txtnorm)

        self.txt_enc = get_text_encoder_hq(opt, opt.embed_size, no_txtnorm=opt.no_txtnorm)
    ...
    def forward_emb(self, images, captions, img_ids, lengths, image_lengths=None):
        ...
        if torch.cuda.is_available():
            ...
            image_lengths = image_lengths.cuda(device)
        ...
        img_out = img_embs.unsqueeze(1)
        txt_out = cap_embs.unsqueeze(1)
    ...
    def eval_emb(self, images, captions, img_ids, lengths, image_lengths=None):
    ...
    def train_emb(self, images, image_lengths, captions, lengths, ids, img_ids, repeat, *args):
        ...
        img_out, txt_out, cp_img_out, cp_txt_out = self.forward_emb(images, captions, img_ids, lengths, image_lengths)
        # print('****************')
        # print(images.shape)
